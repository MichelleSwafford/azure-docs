...
3. Add the groups to the feature to enable *pass-through authentication* and *seamless SSO*. To avoid a UX time-out, ensure that the security groups contain no more than 200 members initially.
   ...
   >When adding a group, users in the group will be updated to use managed authentication immediately. When editing a user in the group (add a user or remove), it can 	take up to 24 hours for the user to get updated from federated to managed. 
