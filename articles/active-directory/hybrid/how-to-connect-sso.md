...
For Windows 10, it is recommended to use SSO via primary refresh token (PRT) . For windows 7 and 8.1 it is recommended to use Seamless SSO.
Seamless SSO needs the user's device to be **domain-joined** and it is not used on Windows 10 [Azure AD joined devices](concept-azure-ad-join.md) or [hybrid Azure AD joined devices](concept-azure-ad-join-hybrid.md). SSO on Azure AD joined, Hybrid Azure AD joined, and Azure AD registered devices works based on the [Primary Refresh Token (PRT)](concept-primary-refresh-token.md)
...
For more information on how SSO works with Windows 10 using PRT, see: [Primary Refresh Token (PRT) and Azure AD](concept-primary-refresh-token.md)
